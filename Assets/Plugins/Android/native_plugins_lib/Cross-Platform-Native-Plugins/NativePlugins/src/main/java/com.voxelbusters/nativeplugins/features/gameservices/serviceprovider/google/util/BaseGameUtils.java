...
public class BaseGameUtils
{
	...
	/**
	 * Show an {@link android.app.AlertDialog} with an 'OK' button and a
	 * message.
	 * 
	 * @param activity
	 ...
	 * @param client
	 *            the GoogleAPIClient instance of the Activity.
	 * @param result
	 *            the ConnectionResult received by the Activity.
	 * @param requestCode
	 *            a request code which the calling Activity can use to identify
	 *            the result of this resolution in onActivityResult.
	 * @param fallbackErrorMessage
	 *            a generic error message to display if the failure cannot be
	 *            resolved.
	 ...
	 * replacing the placeholder IDs. Shows alert dialogs to notify about
	 * problems. DO NOT call this method from a production app, it's meant only
	 * for samples!
	 * 
	 * @param resIds
	 *            the resource IDs to check for placeholders
	...
	public static boolean verifySampleSetup(Activity activity, int... resIds)
	{
		...
		for (int i : resIds)
		{
			if (activity.getString(i).toLowerCase(Locale.ENGLISH).contains("replaceme"))
			{
				problemFound = true;
	...
	/**
	 * Show a {@link android.app.Dialog} with the correct message for a
	 * connection error.
	 * 
	 * @param activity
	 *            the Activity in which the Dialog should be displayed.
	 ...
	 * @param actResp
	 *            the response code from onActivityResult.
	 * @param errorDescription
	 *            the resource id of a String for a generic error message.
	 */
	public static void showActivityResultError(Activity activity, int requestCode, int actResp, int errorDescription)
	{
		...
		Debug.error(CommonDefines.GAME_SERVICES_TAG, "Activity :" + activity + " Request Code : " + requestCode + " Error Description : " + errorDescription);

		switch (actResp)
		{
			case GamesActivityResultCodes.RESULT_APP_MISCONFIGURED:
				errorDialog = makeSimpleDialog(activity, activity.getString(R.string.gameservices_app_misconfigured));
				break;
			case GamesActivityResultCodes.RESULT_SIGN_IN_FAILED:
				errorDialog = makeSimpleDialog(activity, activity.getString(R.string.gameservices_sign_in_failed));
				break;
			case GamesActivityResultCodes.RESULT_LICENSE_FAILED:
				errorDialog = makeSimpleDialog(activity, activity.getString(R.string.gameservices_license_failed));
				break;
			default:
	...
	/**
	 * Create a simple {@link Dialog} with an 'OK' button, a title, and a
	 * message.
	 * 
	 * @param activity
	 *            the Activity in which the Dialog should be displayed.
	...
	{
		return (new AlertDialog.Builder(activity)).setTitle(title).setMessage(text).setNeutralButton(android.R.string.ok, null).create();
	}
